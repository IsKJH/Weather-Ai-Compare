...
$logFile    = "C:\proj\weather-ai-compare\logs\gemini.log"
$metaFile   = "C:\proj\weather-ai-compare\logs\gemini_meta.json"
$prompt     = Get-Content "C:\proj\weather-ai-compare\prompt.txt" -Raw

Set-Location $projectDir

$start = Get-Date
Write-Host "[Gemini] 시작: $start"

gemini -p $prompt --yolo 2>&1 | Tee-Object -FilePath $logFile

$end = Get-Date
$elapsed = ($end - $start).TotalSeconds
Write-Host "[Gemini] 종료: $end (소요: $elapsed 초)"

# 토큰 파싱
$logContent = Get-Content $logFile -Raw
$inputTokens  = if ($logContent -match 'input_tokens["\s:]+([0-9]+)')       { $matches[1] } `
           elseif ($logContent -match 'prompt_tokens["\s:]+([0-9]+)')       { $matches[1] } `
           elseif ($logContent -match 'Input[:\s]+([0-9,]+)\s*tokens')      { $matches[1] -replace ',','' } `
           else { "N/A" }
$outputTokens = if ($logContent -match 'output_tokens["\s:]+([0-9]+)')      { $matches[1] } `
